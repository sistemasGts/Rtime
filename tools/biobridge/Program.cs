        ...
        static void Handle(HttpListenerContext ctx)
        {
            try
            {
                ...
                else if (ctx.Request.HttpMethod == "POST" && ctx.Request.Url.AbsolutePath.TrimEnd('/') == "/compare")
                {
                    ...
                    // DEBUG: volcar cuerpo y cabeceras recibidas para depuración
                    try
                    {
                        var dbgPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "last_compare_request.txt");
                        var sbLog = new StringBuilder();
                        sbLog.AppendLine("--- REQUEST HEADERS ---");
                        sbLog.AppendLine("ContentType: " + (ctx.Request.ContentType ?? "(null)"));
                        foreach (var key in ctx.Request.Headers.AllKeys)
                        {
                            sbLog.AppendLine(key + ": " + ctx.Request.Headers[key]);
                        }
                        sbLog.AppendLine("--- REQUEST BODY ---");
                        sbLog.AppendLine(body ?? "(empty)");
                        File.WriteAllText(dbgPath, sbLog.ToString(), Encoding.UTF8);
                    }
                    catch { }
